...
This project investigates how spatial and geometric information, presented as a graph attention network (GAT), given to a fine tuned YOLOv11m model (YOLO) improves its classification accuracy and ability to reject false positives (GAT corrects the YOLO model on its false detection of components by reclassifying them as background). This investigation is motivated by the notion that PCBs can be interpreted as graphs as the relative locations of components are logically placed to serve a purpose, and hence, follow a trend.
...
If you want to pre-process the data. Download the Kaggle and Roboflow datasets and store them locally as `datasets/kaggle_dataset` and `datasets/roboflow_dataset` respectively and run the `utils/build_dataset.py` script.
