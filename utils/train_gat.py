"""
train_gat.py — Edge-conditioned GATv2 that refines YOLO's per-node class using
graph context. Single model, one primary run.

The head is a gated residual over YOLO: it starts from the logit of YOLO's own
class vector and nudges it using the graph, so the GAT augments the detector
rather than replacing it.

Outputs (in --out, default results/):
  <tag>_confusion.png     node-classification confusion matrix (validation)
  <tag>_curves.png        train vs val accuracy and loss
...
Usage:
  python train_gat.py --train graphs/train.pt --val graphs/valid.pt \
      --names data/data.yaml --epochs 200 --tag gat_edge
"""
...
def to_pyg(path):
    ...
    ds = [Data(x=g["x"], edge_index=g["edge_index"], edge_attr=g["edge_attr"],
...
def evaluate(model, loader, dev, nc, w):
    ...
    for b in loader:
        ...
        n += len(b.y)
        Y.append(b.y.cpu()); P.append(logits.argmax(1).cpu()); B.append(b.yolo_probs.argmax(1).cpu())
...
def plot_yolo_vs_gat(y_true, gat_pred, yolo_pred, names, nc, out):
    ...
    for c in present:
        m = y_true == c
        yo.append((yolo_pred[m] == c).mean()); ga.append((gat_pred[m] == c).mean())
...
def main():
    ...
    for ep in range(a.epochs):
        ...
        if mv["acc"] > best_acc:
            ...
            torch.save({"state": best_state, "epoch": ep, "nc": nc,
                        "in_dim": tr[0].x.shape[1]}, outdir / f"{a.tag}_best.pt")

        if ep % 20 == 0 or ep == a.epochs - 1:
            print(f"  ep{ep:3d} | train acc {mt['acc']*100:5.2f} | "
                  f"val acc {mv['acc']*100:5.2f} loss {mv['loss']:.3f}")

    # final: reload best, evaluate on val, make the plots
